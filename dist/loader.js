export class Loader {
    ...
    start() {
        ...
        if (!appLoader)
            throw new Error("Missig app-loader id in document.");
        appLoader.innerHTML = '<div id="nprogress"><div class="bar" role="bar"><div class="peg"></div></div><div class="spinner" role="spinner"><div class="spinner-icon"></div></div></div>';
        let amount = 0;
    ...
    pushBar(progress) {
        ...
        if (!nprogress)
            return;
        const bar = nprogress.getElementsByClassName('bar')[0];
